 ...
 *
 * ★ 2026-09-12 修复记录（改后缀为 .ino 后可与同目录 DO_NOT_TOUCH_* 头
 *   文件单独编译，作为独立成品固件使用）：
 *   1) parse_tia_range 前缀误匹配：旧版 "100k"/"330k" 会命中 "100"/"330"
 *      档（jz100k_100000 实际校准 100Ω 档、r100k 实际切 100Ω 档），
 *      现改为长前缀优先 + 匹配必须终止于 '_' 或串尾；
 *   2) 配合 DNT 层修复：out_freq 失败/空采集/NaN ratio 的测量现在
 *      统一返回错误而不是"成功+无效数据"；
 *   3) 新增手动固定量程命令：m<freq> = 按固定 V/I/TIA 档位测量，
 *      m = 查看当前固定档位，u/i/r 同时作为固定档位的设档入口；
 *      自动量程路径（<freq>/auto/wauto）行为保持不变。
 *
 * ★ 2026-09-13：从 LCR_UI 文件夹独立为 UART_TIAOSHI sketch。该调试程序
 *   直接包含 DO_NOT_TOUCH_lcr_api.h（DNT 头文件内含非 inline 的函数与
 *   全局变量定义），必须与 UI 固件（LCR_UI）分文件夹编译，否则链接时
 *   产生 multiple definition 冲突。
 */
#include <Arduino.h>
...
// ---- ★ 新增（问题4）：手动固定量程测量 ----
// s_pin_* 为 u/i/r 命令写入的固定档位（默认 x1/x1/22Ω，与 DNT 复位默认一致）。
// 自动量程路径（<freq>/auto/wauto，内部会重置并自动选择档位）行为保持不变；
// m<freq> 走本函数，档位完全由用户固定，不做 lcr_reset_all_ranges + 自动量程。
static int8_t s_pin_v = 0, s_pin_i = 0, s_pin_tia = 0;

// 固定量程单频点测量：复用 DNT 核心块，流程与 lcr_measure_point 相同
// （out_freq -> 档位写入 -> settle -> 采集 -> 双音分析 -> 阻抗派生），
// 仅去掉复位+自动量程；74HC595 写入顺序（V→I→TIA）与单次 settle 时序
// 镜像 lcr_reset_all_ranges。本函数与 DNT 头文件同编译单元，可直接使用其内部块。
static int lcr_measure_z_manual(double f_req, LcrZPoint *out) {
    if (!out || !(f_req > 0.0)) return LCR_API_ERR_PARAM;
    // 频带校验：[10, 20800] Hz 内可证明 out_freq 必然拟合成功（带外直接拒绝）
    if (f_req < FREQ_MIN_HZ || f_req > FREQ_MAX_HZ) return LCR_API_ERR_FREQ_OUT_OF_RANGE;
    double f_act = out_freq(f_req, 20, 20);
    if (f_act <= 0.0) return (f_act == -1.0) ? LCR_API_ERR_FREQ_OUT_OF_RANGE
                                             : LCR_API_ERR_FREQ_NO_FIT;
    g_sig_freq = f_act;
    s_hc595_fsel_update();   // ★ 新硬件（v4）输出 1：频率选择随新频率立即切换
    s_mux_voltage_write((uint8_t)s_pin_v);
    g_voltage_range = (VoltageRange)s_pin_v;
    s_mux_current_write((uint8_t)s_pin_i);
    g_current_range = (CurrentRange)s_pin_i;
    s_mux_tia_write(TIA_ENUM_TO_CODE[(uint8_t)s_pin_tia]);
    g_tia_range = (TiaRange)s_pin_tia;
    ...
    lcr_adc_capture(dur);
    if (lcr_adc_count_a() == 0 || lcr_adc_count_b() == 0) return LCR_API_ERR_MEASURE;
    MeasResult m = lcr_tone_analyze(lcr_adc_buf_a(), lcr_adc_count_a(),
                                    lcr_adc_buf_b(), lcr_adc_count_b(),
                                    g_sig_freq, lcr_adc_fs_per_channel());
    if (!(m.ratio_corr > 0.0)) return LCR_API_ERR_MEASURE;   // NaN-proof
    ZDerived z;
    if (!lcr_derive_z(m, z)) return LCR_API_ERR_MEASURE;
    if (!isfinite(z.z_re) || !isfinite(z.z_im)) return LCR_API_ERR_MEASURE;
    out->f_req = f_req;   out->f_act = z.f;
...
static bool parse_tia_range(const char *s, LcrCalItem &item) {
    // ★ bugfix(1)：旧版仅做无边界前缀匹配，"100k"/"330k" 会误命中
    //   "100"/"330" 档（如 jz100k_100000 实际校准的是 100Ω 档并可能把
    //   错误校准写入 NVS，r100k 实际切到 100Ω 档）。
    //   修复：表项按前缀长度降序排列，且匹配后下一个字符必须是 '_' 或串尾。
    static const struct { const char *s; LcrCalItem it; } tbl[] = {
        {"1meg",LCR_CAL_1M},{"330k",LCR_CAL_330K},{"100k",LCR_CAL_100K},
        {"33k",LCR_CAL_33K},{"10k",LCR_CAL_10K},{"3.3k",LCR_CAL_3K3},{"3k3",LCR_CAL_3K3},
        {"1m",LCR_CAL_1M},{"1k",LCR_CAL_1K},
        {"330",LCR_CAL_330},{"100",LCR_CAL_100},{"22",LCR_CAL_22},
    };
    for (auto &e : tbl) {
        size_t l = strlen(e.s);
        if (strncmp(s, e.s, l) == 0 && (s[l] == '_' || s[l] == '\0')) { item = e.it; return true; }
    }
    return false;
}
...
static void handle_line(const char *raw) {
    ...
    if (!strncmp(buf, "auto", 4)) {
        ...
        if (code < 0) { Serial.println("# ERR: use u1|u3|u10|u33"); }
        else if (lcr_api_set_ranges(code,-1,-1) == LCR_API_OK) {
            s_pin_v = code;   // ★ 同步写入 m<freq> 的固定档位
        ...
        int8_t code = !strcmp(buf+1,"1") ? 0 : !strcmp(buf+1,"3") ? 1 :
                      !strcmp(buf+1,"10") ? 2 : !strcmp(buf+1,"33") ? 3 : -1;
        if (code < 0) { Serial.println("# ERR: use i1|i3|i10|i33"); }
        else if (lcr_api_set_ranges(-1,code,-1) == LCR_API_OK) {
            s_pin_i = code;   // ★ 同步写入 m<freq> 的固定档位
            Serial.println("# READY");
        }
        else Serial.println("# ERR");
    }
        ...
        if (parse_tia_range(buf + 1, it)) {
            if (lcr_api_set_ranges(-1,-1,(int8_t)it) == LCR_API_OK) {
                s_pin_tia = (int8_t)it;   // ★ 同步写入 m<freq> 的固定档位
                Serial.println("# READY");
            }
            else Serial.println("# ERR");
        }
        ...
        // ★ 新增（问题4）：m<freq> = 固定 V/I/TIA 档位单频点测量（不做自动量程）；
        //               m     = 查看当前固定档位。设档入口为 u/i/r 命令。
        if (buf[1] == '\0') {
            Serial.printf("# PINNED: V=%s I=%s TIA=%s (set via u/i/r, measure via m<freq>)\n",
                          VOLT_RANGE_NAME[s_pin_v], CURR_RANGE_NAME[s_pin_i],
                          TIA_RANGE_NAME[s_pin_tia]);
        } else {
            char *end = nullptr;
            double f = strtod(buf + 1, &end);
            if (end != buf + 1 && f >= FREQ_MIN_HZ && f <= FREQ_MAX_HZ) {
                LcrZPoint p{};
                int rc = lcr_measure_z_manual(f, &p);
                if (rc == LCR_API_OK) {
                    Serial.printf("# MANUAL [V=%s I=%s TIA=%s]\n",
                                  VOLT_RANGE_NAME[s_pin_v], CURR_RANGE_NAME[s_pin_i],
                                  TIA_RANGE_NAME[s_pin_tia]);
                    print_z(p);
                } else {
                    Serial.printf("# ERR: manual measure failed (%d)\n", rc);
                }
            } else {
                Serial.println("# ERR: use m<freq> (10..20800) or m");
            }
        }
    }
...
void setup() {
    ...
    if (g_lcr_diag) {
        ...
        Serial.println("# cmds: <freq> | auto[N] | w<freq> | wauto[N] | u/i/r ranges | m | m<freq>");
